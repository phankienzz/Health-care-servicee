    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (session.getAttribute("staffAccount") != null) {
            ...
            for (Permission p : list) {
                if (p.getPermissionID() == 17) {
                    canAdd = true;
                }
                if (p.getPermissionID() == 18) {
                    canView = true;
                }
                if (p.getPermissionID() == 19) {
                    canEdit = true;
                }
                if (p.getPermissionID() == 20) {
                    canDelete = true;
                }
            }
            if ((uri.contains("loadmanage") || uri.contains("manage_Service.jsp")) && !canView) {
                res.sendRedirect("errorPermission.jsp");
            }
            if ((uri.contains("addService") || uri.contains("add_Service.jsp")) && !canAdd) {
                res.sendRedirect("errorPermission.jsp");
            }
            if ((uri.contains("loadupdate") || uri.contains("updateservice") 
                    || uri.contains("editstatus") || uri.contains("update_Service.jsp")) && !canEdit) {
                res.sendRedirect("errorPermission.jsp");
            }
        }
